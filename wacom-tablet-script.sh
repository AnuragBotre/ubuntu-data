...
#
#URL for Script :- https://gist.github.com/Deevad/51820854ffd5ea5cd883
#
...
#In order to execute script use below command
#bash wacom-tablet-script.sh
#reason in ubuntu <<< this redirection is not supported for other shells (likes bsh- Bourne-shell or bash- Bash)
...
#Scale means no of digits after decimal points
...
# Precise Mode + Ratio
#echo $Yscreenpix" "$Xtabletmaxarea" "$Xscreenpix
...
if [ -f /tmp/wacomscript-memory-tokken ]; then
  ...
  echo "Precision mode"
  echo "xsetwacom set $tabletstylus Area 0 0 $Xtabletmaxarea $Ytabletmaxarea"
  xsetwacom set "$tabletstylus" Area 0 0 "$Xtabletmaxarea" "$Ytabletmaxarea"
  echo "xsetwacom set $tableteraser Area 0 0 $Xtabletmaxarea $Ytabletmaxarea"
  xsetwacom set "$tableteraser" Area 0 0 "$Xtabletmaxarea" "$Ytabletmaxarea"
  echo "xsetwacom set $tabletstylus MapToOutput $XtabletactiveareaPIXx$YtabletactiveareaPIX+$XOffsettabletactiveareaPIX+$YOffsettabletactiveareaPIX"  
  
  echo "Map to output X- $XtabletactiveareaPIX"
  echo "Map to output Y- $YtabletactiveareaPIX"
  echo "Map to output XOffsettabletactiveareaPIX - $XOffsettabletactiveareaPIX"
  echo "Map to output YOffsettabletactiveareaPIX - $YOffsettabletactiveareaPIX"
